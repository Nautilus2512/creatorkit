...
export default function TimestampConverter() {
  ...
  const useNow = () => { setInput(String(Math.floor(Date.now() / 1000))); setError("") }
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {rows.length === 0 ? (
              ...
              rows.map(({ label, key, value }) => (
                <div key={key} className="flex items-center justify-between rounded-lg border border-border px-4 py-3 gap-3">
