...
export default function ImageGrid() {
  ...
  return (
    <>
      ...
      />
    <div className="flex h-full flex-col gap-3 p-4" role="main" aria-label="Image Grid tool">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Image Grid / Collage</h2>
          <p className="text-muted-foreground">Arrange images in a grid and export as a single PNG. Press ? for shortcuts.</p>
        </div>
        <Button 
          onClick={download} 
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-3 sm:gap-4" role="group" aria-label="Grid settings">
        <div className="flex items-center gap-2 flex-wrap" role="group" aria-labelledby="layout-label">
          <Label className="text-xs text-muted-foreground shrink-0" id="layout-label">Layout:</Label>
          {LAYOUTS.map(l => (
            <button 
              key={l.key} 
              ...
              className={`text-xs px-2 sm:px-3 py-1 rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${layout === l.key ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}>
              {l.label}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2" role="group" aria-labelledby="gap-label">
          <Label className="text-xs text-muted-foreground" id="gap-label">Gap:</Label>
          <Slider 
            value={[gap]} 
          ...
          />
          <span className="text-xs font-mono text-muted-foreground" aria-live="polite">{gap}px</span>
        </div>
        <div className="flex items-center gap-2" role="group" aria-labelledby="bg-label">
          <Label className="text-xs text-muted-foreground" id="bg-label">BG:</Label>
          <input 
            type="color" 
            value={bgColor} 
            onChange={e => { setBgColor(e.target.value); announceToScreenReader(`Background color changed to ${e.target.value}`) }} 
            className="w-6 h-6 sm:w-7 sm:h-7 rounded border border-border cursor-pointer p-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
            aria-label={`Background color ${bgColor}`}
          />
        </div>
        <div className="flex items-center gap-2" role="group" aria-labelledby="size-label">
          <Label className="text-xs text-muted-foreground shrink-0" id="size-label">Size:</Label>
          {[800, 1200, 2000].map(s => (
            <button 
              key={s} 
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 flex-1 min-h-0 overflow-y-auto lg:overflow-visible">
        {/* Left — images */}
        <div className="flex flex-col rounded-xl border border-border bg-card min-h-[250px] lg:min-h-0 lg:overflow-hidden" role="region" aria-labelledby="images-panel-label">
          <div className="shrink-0 border-b border-border px-4 py-3 flex items-center justify-between">
            <h3 className="text-sm font-medium" id="images-panel-label">Images ({images.length}/{needed})</h3>
          ...
          {images.length === 0 ? (
            <label className="flex-1 flex flex-col items-center justify-center cursor-pointer border-2 border-dashed border-border m-4 rounded-xl hover:border-primary/50 transition-colors focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2">
              <input 
                type="file" 
                accept="image/*" 
                multiple 
            ...
            <>
              <label className="shrink-0 flex items-center justify-center gap-2 p-3 cursor-pointer border-b border-border hover:bg-muted/30 transition-colors focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2">
                <input 
                  type="file" 
                  accept="image/*" 
                  multiple 
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1 max-h-[40vh] lg:max-h-none" role="list" aria-label="Uploaded images">
        ...
        {/* Right — Preview */}
        <div className="flex flex-col rounded-xl border border-border bg-card min-h-[300px] lg:min-h-0 lg:overflow-hidden" role="region" aria-labelledby="preview-panel-label">
          <div className="shrink-0 border-b border-border px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex-1 flex items-center justify-center p-4 sm:p-6 bg-muted/10 overflow-hidden min-h-[250px]" role="img" aria-label={images.length > 0 ? "Image grid preview" : "Empty preview"}>
            {images.length === 0 ? (
