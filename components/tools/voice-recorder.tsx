      ...
      <div className="grid gap-4 md:grid-cols-2 flex-1 min-h-0">
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-border bg-card">
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-5">
            ...
            {isRecording && <p className="text-2xl font-mono font-bold text-destructive tabular-nums">{fmtDur(duration)}</p>}
            {error && <p className="text-xs text-destructive text-center max-w-xs">{error}</p>}
            <Button size="lg" variant={isRecording ? "destructive" : "default"} onClick={isRecording ? stopRecording : startRecording} className="min-w-40">
              {isRecording ? <><Square className="h-5 w-5 mr-2" />Stop</> : <><Mic className="h-5 w-5 mr-2" />Start Recording</>}
            </Button>
          </div>
